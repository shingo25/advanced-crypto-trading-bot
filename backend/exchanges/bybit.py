...
from .base import (
    AbstractExchangeAdapter, OHLCV, FundingRate, OpenInterest, Ticker,
    TimeFrame, ExchangeError, RateLimitError, APIError
)
    ...
    async def fetch_funding_rate(self, symbol: str) -> FundingRate:
        ...
        try:
            ...
            funding_info = await asyncio.get_event_loop().run_in_executor(
                None,
                lambda: self.exchange.fetch_funding_rate(normalized_symbol)
            )
    ...
    async def fetch_open_interest(self, symbol: str) -> OpenInterest:
        ...
        try:
            ...
            oi_info = await asyncio.get_event_loop().run_in_executor(
                None,
                lambda: self.exchange.fetch_open_interest(normalized_symbol)
            )
    ...
    async def fetch_ticker(self, symbol: str) -> Ticker:
        ...
        try:
            ...
            ticker_data = await asyncio.get_event_loop().run_in_executor(
                None,
                lambda: self.exchange.fetch_ticker(normalized_symbol)
            )
