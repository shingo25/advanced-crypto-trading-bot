...
def create_user(username: str, password_hash: str, role: str = "viewer") -> Dict[str, Any]:
    ...
    try:
        ...
        profile = db.profiles_model.create_profile(
            user_id=user_id,
            username=username
        )
...
def update_user(user_id, **kwargs) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        updatable_fields = {k: v for k, v in kwargs.items() 
                           if k in ['username', 'role']}
        
        if updatable_fields:
            updated_profile = db.profiles_model.update_profile(user_id, **updatable_fields)
            if updated_profile:
                return {
