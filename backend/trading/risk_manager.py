...
class RiskManager:
    ...
    def check_position_risk(self, positions: Dict[str, Position], 
                          current_pnl: float) -> List[str]:
        """ポジションリスクをチェック"""
        if not self.is_enabled:
            return []
    ...
    def should_emergency_stop(self, positions: Dict[str, Position], 
                            current_pnl: float) -> bool:
        """緊急停止が必要かチェック"""
        if not self.is_enabled:
            return False
    ...
    def _check_position_size_limit(self, order: Order, 
    ...
    def _check_leverage_limit(self, order: Order, 
                            current_positions: Dict[str, Position]) -> bool:
        """レバレッジ制限をチェック"""
        # 簡単な実装（実際のレバレッジ計算は複雑）
    ...
    def _check_correlation_limit(self, order: Order, 
                               current_positions: Dict[str, Position]) -> bool:
        """相関制限をチェック"""
        # 簡単な実装（実際の相関計算は複雑）
        # 同じ市場での過度の集中を防ぐ
        ...
        if order.symbol.endswith('USDT') or order.symbol.endswith('BTC'):
            similar_positions = [pos for pos in current_positions.values() 
                               if pos.symbol.endswith('USDT') or pos.symbol.endswith('BTC')]
            
            if len(similar_positions) > 3:  # 3つ以上の類似ポジション
                logger.warning(f"Correlation limit exceeded: too many similar positions")
                self.stats['correlation_violations'] += 1
                return False
        
        return True
    
    def _check_portfolio_heat(self, order: Order, 
                            current_positions: Dict[str, Position]) -> bool:
        """ポートフォリオヒートをチェック"""
        # 各ポジションのリスクを合計
        total_risk = 0.0
    ...
    def get_risk_metrics(self, positions: Dict[str, Position], 
        ...
        return {
            ...
            'risk_limit_usage': {
                'max_position_size': max(pos.get_market_value() / self.risk_limits['max_position_size'] 
                                       for pos in positions.values()) if positions else 0.0,
