...
class GlassnodeClient:
    ...
    async def get_whale_flow(self, 
                           symbol: str = "BTC",
                           since: Optional[datetime] = None,
                           until: Optional[datetime] = None) -> List[WhaleFlowData]:
        """ホエールフローデータを取得"""
        params = {
            'a': symbol,
            'f': 'JSON',
    ...
    async def get_nvt_data(self, 
                          symbol: str = "BTC",
                          since: Optional[datetime] = None,
                          until: Optional[datetime] = None) -> List[NVTData]:
        """NVTデータを取得"""
        params = {
            'a': symbol,
            'f': 'JSON',
        ...
        try:
            nvt_data = await self._make_request(
                'indicators/nvt',
                params
            )
            
            nvt_signal_data = await self._make_request(
                'indicators/nvts',
                params
            )
    ...
    async def get_nupl_data(self, 
                           symbol: str = "BTC",
                           since: Optional[datetime] = None,
                           until: Optional[datetime] = None) -> List[NUPLData]:
        """NUPLデータを取得"""
        params = {
            'a': symbol,
            'f': 'JSON',
        ...
        try:
            nupl_data = await self._make_request(
                'indicators/nupl',
                params
            )
            
            supply_profit_data = await self._make_request(
                'indicators/supply_profit_relative',
                params
            )
...
class CryptoQuantClient:
    ...
    async def get_miner_outflow(self, 
                               symbol: str = "BTC",
                               since: Optional[datetime] = None,
                               until: Optional[datetime] = None) -> List[MinerOutflowData]:
        """マイナーアウトフローデータを取得"""
        params = {
            'symbol': symbol,
            'window': '1h'
...
class OnChainDataCollector:
    ...
    async def collect_whale_flows(self, 
                                 symbols: List[str] = None,
                                 since: Optional[datetime] = None) -> Dict[str, List[WhaleFlowData]]:
        """ホエールフローデータを収集"""
    ...
    async def collect_nvt_data(self, 
                              symbols: List[str] = None,
                              since: Optional[datetime] = None) -> Dict[str, List[NVTData]]:
        """NVTデータを収集"""
        if not self.glassnode_client:
            logger.warning("Glassnode client not initialized")
            return {}
    ...
    async def collect_miner_outflows(self, 
                                    symbols: List[str] = None,
                                    since: Optional[datetime] = None) -> Dict[str, List[MinerOutflowData]]:
        """マイナーアウトフローデータを収集"""
        ...
        for flow in whale_flows:
            try:
                db.execute(
                    ...
                    VALUES (?, ?, ?, ?, ?)
                    """,
                    [flow.timestamp, flow.symbol, flow.inflow, flow.outflow, flow.net_flow]
                )
            except Exception as e:
        ...
        for outflow in outflows:
            try:
                db.execute(
                    ...
                    VALUES (?, ?, ?, ?)
                    """,
                    [outflow.timestamp, outflow.symbol, outflow.outflow_amount, outflow.outflow_value_usd]
                )
            except Exception as e:
