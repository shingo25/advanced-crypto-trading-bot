...
class DataCollector:
    ...
    async def collect_batch_ohlcv(self, 
                                 symbols: List[str],
                                 timeframes: List[TimeFrame],
                                 since: Optional[datetime] = None) -> Dict[str, Dict[str, List[OHLCV]]]:
        ...
        for symbol in symbols:
            ...
            for timeframe in timeframes:
                task = asyncio.create_task(
                    self.collect_ohlcv(symbol, timeframe, since)
                )
    ...
    async def _save_ohlcv_to_parquet(self, 
                                    symbol: str,
                                    timeframe: TimeFrame,
