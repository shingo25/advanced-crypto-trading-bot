...
class EMAStrategy(BaseStrategy):
    ...
    def calculate_indicators(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        data["cross_below"] = (data["ema_fast"] < data["ema_slow"]) & (
            data["ema_fast"].shift(1) >= data["ema_slow"].shift(1)
        )
    ...
    def generate_signals(self, data: pd.DataFrame) -> List[Signal]:
        ...
        # ショートエントリー条件（オプション）
            ...
            current["cross_below"]
            and current["downtrend"]
            and current["volume_filter"]
            and not self.state.is_long
            and not self.state.is_short
            and self.parameters.get("allow_short", False)
        ):
            signal = Signal(
                timestamp=current_time,
                symbol=self.symbol,
    ...
    def _backtest_with_params(self, data: pd.DataFrame, params: Dict[str, Any], metric: str) -> float:
        ...
        for i in range(len(temp_data)):
            ...
            # ロングイグジット
                ...
                row["cross_below"]
                or row["close"] <= entry_price * (1 - params["stop_loss_pct"])
                or row["close"] >= entry_price * (1 + params["take_profit_pct"])
            ):
