...
class PortfolioOptimizer:
    ...
    def _generate_random_weights(self, assets: List[str], constraints: List[OptimizationConstraint]) -> Optional[Dict[str, float]]:
        """制約を満たすランダムな重みを生成"""
        import random
    ...
    def _calculate_portfolio_volatility(self, weights: Dict[str, float], covariance_matrix: Dict[str, Dict[str, float]]) -> float:
        """ポートフォリオのボラティリティを計算"""
        variance = 0.0
    ...
    def _calculate_sharpe_ratio(self, weights: Dict[str, float], expected_returns: Dict[str, float], covariance_matrix: Dict[str, Dict[str, float]]) -> float:
        """シャープレシオを計算"""
        portfolio_return = self._calculate_portfolio_return(weights, expected_returns)
        portfolio_volatility = self._calculate_portfolio_volatility(weights, covariance_matrix)
    ...
    def generate_efficient_frontier(self, 
        ...
        return_range = [min_return + (max_return - min_return) * i / (num_portfolios - 1) for i in range(num_portfolios)]
        ...
        for target_return in return_range:
            ...
            try:
                result = self._optimize_min_volatility_with_return_constraint(
                    assets, expected_returns, covariance_matrix, target_return, constraints
                )
    ...
    def _optimize_min_volatility_with_return_constraint(self, 
                                                       assets: List[str],
                                                       expected_returns: Dict[str, float],
                                                       covariance_matrix: Dict[str, Dict[str, float]],
                                                       target_return: float,
