...
class PositionManager:
    ...
    async def liquidate_high_risk_positions(self, risk_threshold: float = 0.80):
        ...
        # 損失の大きいポジションから決済
        positions_by_loss = sorted(self._positions.items(), key=lambda x: x[1].unrealized_pnl)
...
class EnhancedRiskManager:
    ...
    async def check_order_risk(
        self, order: Order, request_context: Dict = None, portfolio_value: Optional[Decimal] = None
    ...
    async def _execute_emergency_actions(self):
        ...
        try:
            ...
            self.circuit_breaker.trip(
                ...
                {"action": "emergency_stop", "timestamp": datetime.now(timezone.utc).isoformat()},
            )
