...
class SecurityManager:
    ...
    def __init__(self, config: Dict):
        ...
        # レート制限カウンター
        self.rate_limit_counters = defaultdict(lambda: {
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    self._log_audit_event("ip_check_failed", {
                        "ip": ip_address, 
            ...
            self._log_audit_event("ip_check", {
                "ip": normalized_ip,
                "allowed": is_allowed,
    ...
    def check_for_anomalies(self, order: Order, user_id: str, portfolio_value: Optional[Decimal] = None) -> bool:
        ...
        # 異常が検知された場合
        if anomalies:
            self._log_audit_event("anomaly_detected", {
                "user_id": user_id,
                "order_id": order.id,
                "anomalies": anomalies
            })
    ...
    def execute_security_checks(
        ...
        try:
            ...
            self._log_audit_event("security_check_passed", {
                "user_id": user_id,
                "order_id": order.id,
            ...
            self._log_audit_event("security_check_error", {
                "error": str(e),
                "order_id": order.id
            })
    ...
    def _cleanup_rate_limit_counters(self, counters: Dict, current_minute: int, current_hour: int):
        """レート制限カウンターのクリーンアップ"""
        # 1分以上古いエントリを削除
        counters['minute'] = deque(
            [t for t in counters['minute'] if t >= current_minute - 1],
            maxlen=60
        )
        
        # 1時間以上古いエントリを削除
        counters['hour'] = deque(
            [t for t in counters['hour'] if t >= current_hour - 1],
            maxlen=3600
        )
    ...
    def add_ip_to_whitelist(self, ip_address: str):
        ...
        self._log_audit_event("ip_whitelist_updated", {
            "action": "add",
            "ip": normalized_ip
        })
    ...
    def update_rate_limits(self, new_limits: Dict):
        ...
        self._log_audit_event("rate_limits_updated", {
