...
class MockDataSource(DataSourceStrategy):
    ...
    async def get_ticker(self, exchange: str, symbol: str) -> Ticker:
        ...
        # 現在の価格を生成（わずかな変動を加える）
        current_price = self.generator.price_history.get(symbol, base_price * np.random.uniform(0.99, 1.01))
    ...
    async def get_funding_rate(self, exchange: str, symbol: str) -> FundingRate:
        ...
        next_funding_time = now.replace(hour=(now.hour + hours_until_next) % 24, minute=0, second=0, microsecond=0)
