    ...
    def audit_all_api_keys(self) -> Dict[str, Dict]:
        ...
        for exchange in exchanges:
            try:
                ...
                # APIキーの検証
                key_valid, key_errors = self.validate_api_key(api_key, exchange) if api_key else (False, ["Missing API key"])
                secret_valid, secret_errors = self.validate_api_key(secret, exchange) if secret else (False, ["Missing secret"])
