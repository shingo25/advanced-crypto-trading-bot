...
# レート制限インスタンス作成
limiter = Limiter(key_func=get_user_id_or_ip, default_limits=[f"{settings.RATE_LIMIT_PER_MINUTE}/minute"])
...
class EnhancedRateLimiter:
    ...
    def check_and_record_attempt(self, client_id: str, success: bool):
        ...
        if self.is_locked_out(client_id):
            raise HTTPException(
                status_code=429, detail=f"Too many failed attempts. Locked out until {self.lockout_until[client_id]}"
            )
