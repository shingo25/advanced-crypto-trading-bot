...
from .models import (
    PaperWalletModel, PaperWalletTransactionModel, PaperWalletDefaultModel, 
    DatabaseManager
)
...
class PaperWalletService:
    ...
    def initialize_user_wallet(
        ...
        try:
            # 既存ウォレットの確認
            existing_wallets = session.query(PaperWalletModel).filter(
                PaperWalletModel.user_id == user_id
            ).all()
            ...
            # デフォルト設定を取得
            default_config = session.query(PaperWalletDefaultModel).filter(
                and_(
                    PaperWalletDefaultModel.name == default_setting,
                    PaperWalletDefaultModel.is_active == True
                )
            ).first()
    ...
    def get_user_balances(self, user_id: UUID) -> Dict[str, Dict[str, float]]:
        ...
        session = self.db_manager.get_session()
        try:
            wallets = session.query(PaperWalletModel).filter(
                PaperWalletModel.user_id == user_id
            ).all()
    ...
    def get_asset_balance(self, user_id: UUID, asset: str) -> Dict[str, float]:
        ...
        session = self.db_manager.get_session()
        try:
            wallet = session.query(PaperWalletModel).filter(
                and_(
                    PaperWalletModel.user_id == user_id,
                    PaperWalletModel.asset == asset
                )
            ).first()
    ...
    def update_balance(
        ...
        try:
            ...
            # 残高が負になる場合はエラー
            if new_balance < 0:
                logger.warning(f"Insufficient balance: user={user_id}, asset={asset}, current={old_balance}, requested={amount}")
                return False
    ...
    def lock_balance(
        ...
        try:
            ...
            if available < amount:
                logger.warning(f"Insufficient available balance: user={user_id}, asset={asset}, available={available}, requested={amount}")
                return False
    ...
    def unlock_balance(
        ...
        try:
            ...
            if wallet.locked_balance < amount:
                logger.warning(f"Insufficient locked balance: user={user_id}, asset={asset}, locked={wallet.locked_balance}, requested={amount}")
                return False
    ...
    def execute_trade(
        ...
        try:
            ...
            # 3. 手数料を差し引く
            if success and fee_amount > 0 and not self._update_balance_in_session(
            ...
            if success:
                session.commit()
                logger.info(f"Trade executed: user={user_id}, -{sell_amount} {sell_asset}, +{buy_amount} {buy_asset}, -{fee_amount} {fee_asset}")
                return True
            else:
                session.rollback()
    ...
    def _update_balance_in_session(
        ...
        try:
            ...
            # 残高が負になる場合はエラー
            if new_balance < 0:
                logger.warning(f"Insufficient balance in trade: user={user_id}, asset={asset}, current={old_balance}, requested={amount}")
                return False
    ...
    def get_transaction_history(
        ...
        session = self.db_manager.get_session()
        try:
            query = session.query(PaperWalletTransactionModel).filter(
                PaperWalletTransactionModel.user_id == user_id
            )
            ...
            transactions = query.order_by(
                desc(PaperWalletTransactionModel.created_at)
    ...
    def get_default_settings(self) -> List[Dict]:
        ...
        session = self.db_manager.get_session()
        try:
            defaults = session.query(PaperWalletDefaultModel).filter(
                PaperWalletDefaultModel.is_active == True
            ).all()
