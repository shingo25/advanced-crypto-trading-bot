    ...
    def update_order(self, order_id: str, **kwargs) -> bool:
        ...
        try:
            ...
            updatable_fields = [
                ...
                'exchange_order_id', 'submitted_at', 'filled_at', 'cancelled_at',
                'error_message', 'error_code', 'fee_amount', 'fee_currency', 'metadata'
            ]
    ...
    def get_position_summary(self, user_id: UUID, paper_trading: bool = False) -> List[Dict]:
        ...
        try:
            ...
            query = session.query(
                ...
                func.sum(
                    func.case(
                        (TradeModel.side == 'buy', TradeModel.quantity),
                        else_=-TradeModel.quantity
                    )
                ).label('net_quantity'),
                func.avg(
                    func.case(
                        (TradeModel.side == 'buy', TradeModel.price),
                        else_=None
                    )
                ).label('avg_buy_price'),
                func.avg(
                    func.case(
                        (TradeModel.side == 'sell', TradeModel.price),
                        else_=None
                    )
                ...
                TradeModel.exchange, TradeModel.symbol
            ).having(
                func.sum(
                    func.case(
                        (TradeModel.side == 'buy', TradeModel.quantity),
...
class AnalyticsService:
    ...
    def get_trading_performance(
        ...
        try:
            ...
            daily_pnl_query = session.query(
                ...
                func.date(TradeModel.executed_at)
    ...
    def get_risk_metrics(
        ...
        try:
            # 集中度リスク計算
            concentration_query = session.query(
                TradeModel.symbol,
                func.sum(TradeModel.quantity * TradeModel.price).label('symbol_volume')
            ).filter(
                and_(
                    TradeModel.user_id == user_id,
                    TradeModel.paper_trading == paper_trading,
