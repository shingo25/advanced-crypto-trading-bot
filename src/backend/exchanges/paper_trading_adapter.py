...
class PaperTradingAdapter(AbstractExchangeAdapter):
    ...
    async def _lock_balance_for_order(self, order: Order):
        ...
        if order.side == OrderSide.BUY:
            ...
            self.wallet_service.lock_balance(
                UUID(self.user_id), base_asset, required_amount, order.exchange_order_id
            )
    ...
    def _format_order_response(self, order: Order) -> Dict[str, Any]:
        ...
        return {
            ...
            "fee": {
                "amount": float(order.fee_amount) if order.fee_amount else 0.0,
    ...
    def get_transaction_history(self, asset: str = None, limit: int = 100) -> List[Dict]:
        """取引履歴を取得"""
        return self.wallet_service.get_transaction_history(
            UUID(self.user_id), asset=asset, limit=limit
        )