...
class BitgetAdapter(AbstractExchangeAdapter):
    """Bitget取引所アダプタ"""
    ...
    async def fetch_ohlcv(
        ...
        try:
            ...
            logger.info(f"Fetched {len(ohlcv_list)} OHLCV records for {symbol} from Bitget")
            return ohlcv_list
    ...
    async def fetch_funding_rate(self, symbol: str) -> FundingRate:
        ...
        try:
            ...
            funding_rate = FundingRate(
                timestamp=datetime.fromtimestamp(funding_info["timestamp"] / 1000, tz=timezone.utc),
                symbol=normalized_symbol,
                funding_rate=float(funding_info["fundingRate"]),
                next_funding_time=datetime.fromtimestamp(funding_info["fundingTimestamp"] / 1000, tz=timezone.utc),
            )

            logger.info(f"Fetched funding rate for {symbol} from Bitget: {funding_rate.funding_rate}")
            return funding_rate
    ...
    async def fetch_open_interest(self, symbol: str) -> OpenInterest:
        ...
        try:
            ...
            open_interest = OpenInterest(
                timestamp=datetime.fromtimestamp(oi_info["timestamp"] / 1000, tz=timezone.utc),
                symbol=normalized_symbol,
                open_interest=float(oi_info["openInterestAmount"]),
                open_interest_value=float(oi_info.get("openInterestValue", oi_info["openInterestAmount"])),
            )

            logger.info(f"Fetched open interest for {symbol} from Bitget: {open_interest.open_interest}")
    ...
    async def get_balance(self) -> Dict[str, float]:
        ...
        try:
            ...
            logger.info(f"Fetched balance for {len(free_balance)} assets from Bitget")
            return free_balance
    ...
    def normalize_symbol(self, symbol: str) -> str:
        """Bitget用のシンボル正規化"""
        # BTC/USDT -> BTCUSDT または BTC/USDT:USDT (perpetual)
        if "/" in symbol and ":USDT" not in symbol:
            # パーペチュアル契約の場合は :USDT を追加
