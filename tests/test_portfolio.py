...
def test_portfolio_manager_import():
    ...
    try:
        from backend.portfolio.manager import PortfolioManager, Portfolio, Asset, AssetType
...
def test_portfolio_weights():
    ...
    try:
        ...
        # 複数の資産を追加
        assets = [
            Asset(symbol="BTC", asset_type=AssetType.CRYPTO, current_price=50000.0, balance=1.0, target_weight=0.5),
            Asset(symbol="ETH", asset_type=AssetType.CRYPTO, current_price=3000.0, balance=10.0, target_weight=0.3),
            Asset(symbol="USDT", asset_type=AssetType.STABLE, current_price=1.0, balance=20000.0, target_weight=0.2)
        ]
...
def test_rebalance_suggestions():
    ...
    try:
        ...
        # 資産を追加（アンバランスな状態）
        assets = [
            Asset(symbol="BTC", asset_type=AssetType.CRYPTO, current_price=50000.0, balance=1.6, target_weight=0.5),  # 80%
            Asset(symbol="ETH", asset_type=AssetType.CRYPTO, current_price=3000.0, balance=3.33, target_weight=0.3),  # 10%
            Asset(symbol="USDT", asset_type=AssetType.STABLE, current_price=1.0, balance=10000.0, target_weight=0.2)  # 10%
        ]
...
def test_portfolio_optimizer_import():
    ...
    try:
        from backend.portfolio.optimizer import PortfolioOptimizer, OptimizationObjective
