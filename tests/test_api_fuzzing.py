        ...
        def test_no_server_errors(self, case):
            ...
            try:
                response = client.request(
                    method=case.method, url=case.formatted_path, headers=case.headers, params=case.query, json=case.body
                )

                # サーバーエラー（5xx）が発生しないことを確認
                assert (
                    response.status_code < 500
                ), f"Server error on {case.method} {case.formatted_path}: {response.text}"
    ...
    class TestAPIFuzzingAuthentication:
            ...
            try:
                ...
                if "/auth/" in case.formatted_path and case.formatted_path not in ["/auth/login", "/auth/register"]:
                    assert response.status_code in [
                        ...
                        403,
                        404,
                    ], f"Protected endpoint should require auth: {case.formatted_path}"
        ...
        def test_invalid_token_handling(self, case, invalid_auth_token):
            ...
            try:
                ...
                response = client.request(
                    method=case.method, url=case.formatted_path, headers=headers, params=case.query, json=case.body
                )
...
class TestAPIFuzzingInput:
    ...
    def test_xss_attempts(self):
        ...
        for payload in xss_payloads:
            # ユーザー登録でXSSを試行
            register_data = {"username": payload, "email": f"test{payload}@example.com", "password": "ValidPass123"}
            ...
            if response.status_code >= 400:
                ...
                assert response.status_code in [400, 422], "XSS payload should be rejected with validation error"
...
class TestAPIFuzzingBusiness:
    ...
    def test_extreme_price_values(self):
        ...
        for price in extreme_prices:
            # 市場データAPIで極端な価格範囲を試行
            params = {"symbol": "BTC/USDT", "min_price": price, "max_price": price * 2 if price > 0 else 1}
    ...
    def test_server_header_disclosure(self):
        ...
        if server_header:
            for header in sensitive_headers:
                ...
                assert not any(
                    char.isdigit() for char in server_header
