...
class TestPaperLiveModeSelection:
    ...
    def test_default_mode_is_paper(self):
        ...
        adapter = factory.create_adapter("binance", paper_config=paper_config)
        assert isinstance(adapter, PaperTradingAdapter)

        # 明示的にpaperモードを指定
        adapter = factory.create_adapter("binance", trading_mode="paper", paper_config=paper_config)
        assert isinstance(adapter, PaperTradingAdapter)
...
class TestPaperModeTradeIsolation:
    ...
    def test_paper_mode_no_external_api_calls(self, mock_ccxt_binance):
        ...
        # モック注文データ
        order_data = {"symbol": "BTC/USDT", "type": "market", "side": "buy", "amount": 0.001, "price": None}
...
class TestRuntimeSafety:
    ...
    def test_factory_state_isolation(self):
        ...
        paper_config = get_test_paper_config()
        adapter1 = factory1.create_adapter("binance", trading_mode="paper", paper_config=paper_config)
        adapter2 = factory2.create_adapter("bitget", trading_mode="paper", paper_config=paper_config)
