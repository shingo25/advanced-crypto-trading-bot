...
def mock_exchange_adapter():
    ...
    adapter.fetch_ticker = AsyncMock(return_value=Mock(last=45000.0, bid=44990.0, ask=45010.0))
    adapter.get_balance = AsyncMock(return_value={"BTC": 1.0, "USDT": 50000.0, "ETH": 10.0})
    return adapter
...
def mock_account_service():
    """モックアカウントサービス"""
    service = Mock()
    service.get_balance = AsyncMock(return_value={"BTC": 1.0, "USDT": 50000.0, "ETH": 10.0})
    return service
...
def test_request_context():
    """テスト用リクエストコンテキスト"""
    return {"user_id": "test_user_123", "ip_address": "127.0.0.1", "session_id": "session_abc"}
...
class TestFullIntegrationScenarios:
    ...
    async def test_successful_order_creation_flow(
        self, security_config, test_order, test_request_context, mock_exchange_adapter, mock_account_service
    ):
        """正常な注文作成フローテスト"""
        # ファクトリを設定
    ...
    async def test_security_rejection_flow(
        self, security_config, test_request_context, mock_exchange_adapter, mock_account_service
    ):
        """セキュリティ拒否フローテスト"""
        # 禁止IPアドレスに変更
