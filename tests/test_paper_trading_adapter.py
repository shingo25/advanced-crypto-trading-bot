...
class TestPaperTradingAdapterEdgeCases:
    ...
    def test_reset_wallet_functionality(self):
        ...
        # 残高変更を加える（テスト用にダミー操作）
        # ウォレットサマリーを取得してリセット前の状態を確認
        adapter.get_wallet_summary()
    ...
    async def test_concurrent_orders(self):
        ...
        with patch.object(adapter, "get_order_book") as mock_order_book:
            ...
            # 同時実行
            results = await asyncio.gather(*[adapter.place_order(order) for order in orders])
            ...
            # 手数料を考慮して実際に受け取るBTC量を計算
            # 手数料率 0.001 (0.1%) を考慮
            actual_expected_btc = expected_btc * (1 - 0.001)
            assert balances["BTC"]["total"] == pytest.approx(actual_expected_btc, abs=1e-5)
