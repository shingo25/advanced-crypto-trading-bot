...
from src.backend.database.models import PaperWalletModel, PaperWalletTransactionModel, PaperWalletDefaultModel
...
class TestPaperWalletServiceBasic:
    ...
    def test_force_reset_functionality(self):
        ...
        result = self.wallet_service.initialize_user_wallet(
            self.test_user_id, "advanced", force_reset=True
        )
        assert result is True
...
class TestPaperWalletServiceTransactions:
    ...
    def test_update_balance_positive(self):
        ...
        result = self.wallet_service.update_balance(
            self.test_user_id, "USDT", Decimal("50000"), "deposit", 
    ...
    def test_update_balance_negative(self):
        ...
        # 残高を減少
        result = self.wallet_service.update_balance(
            self.test_user_id, "USDT", Decimal("-30000"), "withdraw",
    ...
    def test_insufficient_balance_protection(self):
        ...
        # 残高を超える減少を試行
        result = self.wallet_service.update_balance(
            self.test_user_id, "USDT", Decimal("-150000"), "withdraw"
        )
    ...
    def test_lock_balance_functionality(self):
        """残高ロック機能テスト"""
        # 残高をロック
        result = self.wallet_service.lock_balance(
            self.test_user_id, "USDT", Decimal("30000"), "order_123"
        )
        assert result is True
    ...
    def test_unlock_balance_functionality(self):
        """残高ロック解除機能テスト"""
        # まずロック
        self.wallet_service.lock_balance(
            self.test_user_id, "USDT", Decimal("30000"), "order_123"
        )
        
        # ロック解除
        result = self.wallet_service.unlock_balance(
            self.test_user_id, "USDT", Decimal("30000"), "order_123"
        )
        assert result is True
    ...
    def test_lock_insufficient_balance_protection(self):
        ...
        # 利用可能残高を超えるロックを試行
        result = self.wallet_service.lock_balance(
            self.test_user_id, "USDT", Decimal("150000"), "order_456"
        )
...
class TestPaperWalletServiceDataIntegrity:
    ...
    def test_transaction_history_recording(self):
        ...
        # いくつかの取引を実行
        self.wallet_service.update_balance(
            self.test_user_id, "USDT", Decimal("1000"), "deposit"
        )
        self.wallet_service.update_balance(
            self.test_user_id, "USDT", Decimal("-500"), "withdraw"
        )
        ...
        # 履歴が正しく記録されている
        assert len(history) >= 6  # 初期化4回 + 追加2回
    ...
    def test_concurrent_operations_safety(self):
        ...
        def update_balance():
            result = self.wallet_service.update_balance(
                self.test_user_id, "USDT", Decimal("1000"), "concurrent_test"
            )
...
class TestPaperWalletServiceErrorHandling:
    ...
    def test_invalid_default_setting(self):
        ...
        # 存在しない設定名
        result = self.wallet_service.initialize_user_wallet(
            self.test_user_id, "nonexistent_setting"
        )
        assert result is False
    ...
    def test_operations_on_nonexistent_user(self):
        ...
        # 初期化せずに残高更新
        result = self.wallet_service.update_balance(
            nonexistent_user, "USDT", Decimal("1000"), "deposit"
        )
    ...
    def test_reset_wallet_functionality_service(self):
        ...
        # 残高変更
        self.wallet_service.update_balance(
            self.test_user_id, "USDT", Decimal("-50000"), "test_change"
        )
