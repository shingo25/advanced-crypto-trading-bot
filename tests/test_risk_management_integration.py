...
from src.backend.risk.circuit_breaker import BreakerState, CircuitBreaker, TripReason
from src.backend.trading.enhanced_risk_manager import EnhancedRiskManager, PositionManager
from src.backend.trading.orders.models import Order, OrderSide, OrderType
...
class Position:
    ...
    def __init__(
        self, symbol: str, side: str, quantity: float, entry_price: float, current_price: float, unrealized_pnl: float
    ):
        self.symbol = symbol
        self.side = side
...
def sample_position():
    """サンプルポジション"""
    return Position(
        symbol="BTC/USDT", side="long", quantity=1.0, entry_price=45000.0, current_price=46000.0, unrealized_pnl=1000.0
    )
...
class TestCircuitBreaker:
    ...
    def test_callback_execution(self, circuit_breaker_config):
        ...
        cb.set_callbacks(on_trip=trip_callback, on_reset=reset_callback, notification=notification_callback)
...
class TestEnhancedRiskManager:
    ...
    async def test_order_risk_check_normal(self, enhanced_risk_config, sample_order):
        ...
        is_allowed, error_msg = await erm.check_order_risk(sample_order, portfolio_value=Decimal("100000"))
    ...
    def test_comprehensive_status(self, enhanced_risk_config):
        ...
        required_keys = ["timestamp", "circuit_breaker", "position_manager", "basic_risk", "monitoring"]
