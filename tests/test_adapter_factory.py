...
class TestExchangeFactoryPaperTrading:
    ...
    def test_paper_trading_mode_validation(self):
        ...
        # user_id無しでPaper Tradingを試行（エラーになるはず）
        with pytest.raises(ValueError, match="Paper trading requires user_id"):
    ...
    def test_live_trading_adapter_creation(self, mock_settings):
        """Live Tradingアダプター作成テスト"""
        # モック設定
        mock_settings.BINANCE_API_KEY = "test_api_key"
        mock_settings.BINANCE_SECRET = "test_secret"
        factory = ExchangeFactory()
    ...
    def test_unsupported_exchange_for_paper_trading(self):
        ...
        # 存在しない取引所名でPaper Trading（デフォルトにフォールバック）
        adapter = factory.create_adapter(
